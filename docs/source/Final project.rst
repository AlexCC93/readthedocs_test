...
Once the ``Dockerfile`` is ready, let's generate a Docker image out of it. Go to the Windows terminal and navigate to the folder that contains the ``Dockerfile`` file and execute:
...
Next, this Docker image will be the base for creating a Docker container which the final project programming will be carried on in. To achieve this, follow the instruction on `this section`_  of the course, but running the container with this different command:
